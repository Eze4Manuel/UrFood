...
lib.get = async (page, search, token, user_type) => {
    ...
    try {
        ...
        if (search) {
            uri = `/auth/admin?user_type=${user_type}&page=${page}&q=${search}`;
        } else {
            uri = `/auth/admin?user_type=${user_type}&page=${page}`;
        }
...
lib.updateVendor = async (data, token) => {
    try {
        let cfg = helpers.getHeaderConfig(String(token).substr(7));
        return await (await request.put(`/auth/admin-update-vendor`, data, cfg)).data 
    } catch (e) {
        return {status: 'error', msg: e?.response?.data?.msg || e?.message}
    }
}
